...
typedef struct{
  ...
  int bs_vpno;				/* starting virtual page number */
  int bs_npages;			/* number of pages in the store */
  struct bs_next_proc* next_entry;
 // int bs_sem;				/* semaphore mechanism ?	*/
} bs_next_proc;
...
//extern int create_vmemlist(int pid);
...
//void set_frm_tab_upon_frame_allocation(int pid, int frame_no, int frame_type);

SYSCALL free_frm(int i);


extern unsigned long ctr1000;
extern struct fifo_list* fr_list_ptr;
extern struct class_queue* nru_head;
extern pt_t* global_page_table[]; 
