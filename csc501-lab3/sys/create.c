					...
					/* array in the code)		*/
{
	...
	//kprintf("\nInside CREATE(): allocating new page directory");
	page_dir = create_page_dir(pid);
	if( page_dir == NULL)
	{
		restore(ps);
		return(SYSERR);
	...
	pptr->pdbr = (unsigned long)page_dir/NBPG;
	//kprintf("\nInside CREATE(): PDBR Given %d", pptr->pdbr);
		/* Bottom of stack */
	*saddr = MAGIC;
