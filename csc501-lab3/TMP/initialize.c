...
int aTail, aHead, bTail, bHead, cTail, cHead, cTail, cHead;
char 	vers[80];
...
sysinit()
{
	...
	//unsigned int pdbr = (FRAME0)*NBPG;
	//kprintf("\n PDBR value : %32b, %d ", pdbr, pdbr);
	//write_cr3(pdbr);
...
*/
	...
	page_dir = create_page_dir(0);
	if( page_dir==NULL)
	{
		return(SYSERR);
	}
	
	//kprintf("\n Page Dir address for null is %d and page dir frame no is %d", (unsigned int)page_dir, (unsigned int)page_dir, ((unsigned int)page_dir/NBPG));
	//temp = (unsigned int)page_dir/NBPG;
	
	
	//kprintf("\n temp value after shifting left 12 bits %d ", (unsigned int)temp);
	
	unsigned int pdbr = (unsigned int)page_dir/NBPG;
	temp = pdbr<<12;
	//kprintf("\n PDBR value : %32b, %d , temp %32b	%d", pdbr, pdbr, temp, temp);
	write_cr3(temp);


	set_evec(14, (unsigned long)page_fault_isr);
	
	//kprintf("\n Paging going to enable");
	enable_paging();
	//kprintf("\n Paging enabled");
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	aTail = 1 + (aHead=newqueue());
	
/*	bTail = 1 + (bHead=newqueue());
	
	cTail = 1 + (cHead=newqueue());
	
	dTail = 1 + (dHead=newqueue());*/
	 //bTail, bHead, cTail, cHead, cTail, cHead;
